package com.hsy.ai.web.chat.aop;
...
import com.hsy.ai.service.common.thread.LogThread;
...
@Aspect
@Component
public class AiChatLogAspectJ {
    private final static Logger _logger = LoggerFactory.getLogger(AiChatLogAspectJ.class) ;
     ...
     */
    @Pointcut("@annotation(com.hsy.ai.base.annotation.AspectJLogAnnotation)")
    ...
    private void saveLogToDB(ProceedingJoinPoint joinPoint, AspectJLogAnnotation aspectJLog, long timing, Object[] args, Object obj, Throwable e) throws Throwable {
        ...
        if(null!=aspectJLog&&aspectJLog.saveToDb()){
            ...
            try{
                request = ConstantsUtils.requestThreadLocal.get() ;
            }catch(Exception e1){
             ...
             */
            fixedThreadPool.getFixedThreadPool().execute(new LogThread(log,iTAiLogService)) ;
        }
    }
